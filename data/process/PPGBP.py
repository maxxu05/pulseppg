...
class PPGDataProcessor:
    def __init__(self, download_dir: str, fs_target: int):
        ...
        ## this was added in order to match papagei's splits
    ...
    def process_data(self):
        self.df = pd.read_excel(f"{self.download_dir}/Data File/PPG-BP dataset.xlsx", header=1)
        subjects = self.df.subject_ID.values
        main_dir = f"{self.download_dir}/Data File/0_subject/"
        ppg_dir = f"{self.download_dir}/Data File/ppg/"
    ...
    def split_and_save_labels(self):
        ...
        df_train.to_csv(f"{self.download_dir}/Data File/train.csv", index=False)
        df_val.to_csv(f"{self.download_dir}/Data File/val.csv", index=False)
        df_test.to_csv(f"{self.download_dir}/Data File/test.csv", index=False)
    ...
    def process_additional_data(self, ppg_dir):
        for name, ids in zip(["train", "test", "val"], [self.train_ids, self.test_ids, self.val_ids]):
            ...
            np.save(os.path.join(self.download_dir, f"{name}_X_ppg_{self.fs_target}Hz"), data)
            np.save(os.path.join(self.download_dir, f"{name}_y_sysbp"), label_sysbp)
            np.save(os.path.join(self.download_dir, f"{name}_y_diasbp"), label_diasbp)
            np.save(os.path.join(self.download_dir, f"{name}_y_hr"), label_hr)
            np.save(os.path.join(self.download_dir, f"{name}_y_ht"), label_ht)
            ...
            np.save(os.path.join(self.download_dir, f"{name}_avgsegamt"), label_averagesegmentamt)
            np.save(os.path.join(self.download_dir, f"{name}_y_sysbp_patient"), label_sysbp_patient)
            np.save(os.path.join(self.download_dir, f"{name}_y_diasbp_patient"), label_diasbp_patient)
            np.save(os.path.join(self.download_dir, f"{name}_y_hr_patient"), label_hr_patient)
            np.save(os.path.join(self.download_dir, f"{name}_y_ht_patient"), label_ht_patient)

def main(newhz: int, download_dir: str):
    processor = PPGDataProcessor(download_dir=download_dir, fs_target=newhz)
    processor.process_data()

if __name__ == "__main__":
    download_dir = "../../data/ppgbp"
    main(newhz=50, download_dir=download_dir)
    main(newhz=125, download_dir=download_dir)
