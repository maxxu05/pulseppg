"""
PPG Preprocessing and Denoising Module

This module is designed for downloading, extracting, and preprocessing PPG (Photoplethysmogram) files 
from the WESAD dataset. It includes functionalities for denoising PPG data using a bandpass filter, 
FFT reconstruction, and resampling to create subsequences suitable for machine learning tasks. 
Furthermore, it saves the processed data into numpy arrays for easy access.

Functions:
- downloadextract_PPGfiles: Downloads and extracts PPG data from a remote source.
- preprocess_PPGProcesses PPG data, denoises signals, and creates subsequences for ML.
- download_file: Utility function for downloading files with a progress bar.
- denoisePPG: Class that implements denoising methods for PPG signals.
"""
...
# local resampling utility; the project should provide this function
...
def preprocess_PPGdata(ppgpath: str = "data/ppg", processedppgpath: str = "data/ppg/binary", reprocess: bool = False) -> None:
    """
    Preprocess PPG data from the WESAD dataset.
    ...
    # Find minimum length of PPG and labels; they are differnet lengths since PPG is 64 hz and labels are 700 hz
    minlengthofppg = min(len(ppg) for ppg in ppgs)
    minlengthoflabels = min(len(label) for label in labels)

    # Truncate data to match minimum PPG and labels length for consistency
    ...
    # Create subsequences of PPG and labels for ML training
    ...
    subseq_size_data = 50 * 60    # Pulse-PPG is trained with PPG signal of 50 Hz; hence WESAD PPG signal is resampled at 50 Hz
    ...
    for patient, label in enumerate(labels_original):
        ...
        for unique, unique_startidx in zip(uniques, uniques_index):
            flag = False
            if unique not in [1, 2, 3]:  # Only keep certain labels
                continue
            ...
            while True:
                ...
                # Stack the data for subsequences
